...
def resolve_google_news_url(url, headers):
    """Resolve Google News URLs to their original article URL when possible"""
...
def scrape_google_news():
    """Scrape global news (GDELT) from LAST UPDATE DATE to now (incremental collection)"""
    all_articles = []
    new_articles = []
    seen_urls = set()
    ...
    # Load existing articles for accumulation
    ...
    # Get the last update time to avoid re-scraping old data
        ...
        try:
            with open(meta_path, 'r', encoding='utf-8') as meta_file:
                ...
                if last_updated:
                    ...
                    start_date = datetime.now() - timedelta(hours=1)  # Default: last 1 hour
        except Exception:
            ...
            if articles:
                ...
                for item in items:
                    try:
                        ...
                        # Check if article is relevant to Australia migration/employment
                        if not is_australia_relevant(title_text):
                            continue
    ...
    if all_articles:
        ...
        oldest = min(dates)
        newest = max(dates)
        print(f"📅 Database coverage: {oldest.strftime('%Y-%m-%d')} to {newest.strftime('%Y-%m-%d')}")
